...
export class CustomizationCenter {
  ...
  #appearance(config: CustomizationConfig): void {
    ...
    const form = element("form", "moon-custom-form"); const name = element("input", "moon-settings-input"); name.placeholder = "Nome do tema"; name.setAttribute("aria-label", "Nome do novo tema"); const save = button("moon-primary-button", "Salvar tema completo", "plus"); save.type = "submit"; save.append(element("span", "", "Salvar tema")); form.append(name, save); form.addEventListener("submit", event => { event.preventDefault(); try { this.options.store.saveTheme(name.value || `Tema ${this.options.store.document.themes.length + 1}`); this.#say("Tema completo salvo."); this.#render(); } catch (error) { this.#error(error); } }); themes.append(form);
