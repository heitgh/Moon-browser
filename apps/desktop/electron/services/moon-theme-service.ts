...
export class MoonThemeService {
  ...
  async apply(id: string): Promise<{ readonly summary: MoonThemeSummary; readonly tokens: MoonThemeTokens; readonly wallpaperData?: string; readonly iconData?: Readonly<Partial<Record<"logo" | "newTab" | "privateTab", string>>> }> {
    const record = await this.#record(id); const tokens = JSON.parse(record.tokens) as MoonThemeTokens;
    return { summary: this.#summary(record), tokens, ...(await this.#wallpaperData(record, tokens)), ...(await this.#iconData(record, tokens)) };
  }
