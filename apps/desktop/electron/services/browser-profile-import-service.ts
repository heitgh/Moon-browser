...
export class BrowserProfileImportService {
  ...
  async import(selection: ImportSelection): Promise<ImportResult> {
    ...
    const selected: ImportedProfileData = {
      bookmarks: selection.categories.includes("bookmarks")
        ? all.bookmarks
        : [],
  ...
  async importBookmarksHtml(): Promise<ImportResult | null> {
    ...
    if (!info.isFile() || info.size > 20_000_000)
      throw new Error("Bookmarks HTML exceeds 20 MB");
    ...
    for (
      ...
      match && bookmarks.length < MAX_IMPORT_ITEMS;
      match = pattern.exec(html)
    ) {
      const url = safeWebUrl(match[1]);
      if (!url) continue;
      const title =
        sanitizeImportedBookmarkTitle(match[2] ?? "").slice(0, 2_000) || url;
      bookmarks.push({
        id: importedId("html", url),
        title,
        url,
        time: info.mtimeMs,
      });
    }
    return this.persistence.importExternalProfile(
      `html-${createHash("sha256").update(path).digest("hex").slice(0, 16)}`,
      { bookmarks, history: [] },
    );
  }
  ...
  async #addSource(
    ...
    try {
      ...
      const modifiedAt = Math.max(
        ...(await Promise.all(
          ["Bookmarks", "History", "places.sqlite"].map(async (file) => {
            try {
              return (await stat(join(directory, file))).mtimeMs;
            } catch {
              return 0;
            }
          }),
  ...
  async #readSource(
    source: Pick<ImportSource, "directory" | "kind">,
  ): Promise<ImportedProfileData> {
    return source.kind === "chromium"
      ? this.#readChromium(source.directory)
      : this.#readFirefox(source.directory);
  }
  ...
  async #readChromium(directory: string): Promise<ImportedProfileData> {
    ...
    const history = await readStagedDatabase(
      join(directory, "History"),
      [] as readonly ImportedLink[],
      async (database) => {
        const rows = await database.all<{
          readonly url: string;
          readonly title: string;
          readonly last_visit_time: number;
        }>(
        ...
        return rows.flatMap((row) => {
          ...
          const time = Math.max(
            0,
            Math.floor(
              Number(row.last_visit_time) / 1_000 - CHROMIUM_EPOCH_OFFSET_MS,
            ),
          );
          return [
            {
              id: importedId("history", url),
              title: row.title.slice(0, 2_000) || url,
              url,
              time,
            },
          ];
  ...
  async #readFirefox(directory: string): Promise<ImportedProfileData> {
    return readStagedDatabase(
      ...
      { bookmarks: [], history: [] } as ImportedProfileData,
      async (database) => {
        const bookmarkRows = await database.all<{
          readonly url: string;
          readonly title: string | null;
          readonly dateAdded: number;
        }>(
          "SELECT p.url, COALESCE(b.title, p.title) AS title, b.dateAdded FROM moz_bookmarks b JOIN moz_places p ON p.id = b.fk WHERE b.type = 1 AND b.fk IS NOT NULL ORDER BY b.dateAdded DESC LIMIT ?",
          [MAX_IMPORT_ITEMS],
        );
        const historyRows = await database.all<{
          readonly url: string;
          readonly title: string | null;
          readonly last_visit_date: number | null;
        }>(
          "SELECT url, title, last_visit_date FROM moz_places WHERE last_visit_date IS NOT NULL ORDER BY last_visit_date DESC LIMIT ?",
          [MAX_IMPORT_ITEMS],
        );
        const bookmarks = bookmarkRows.flatMap((row) =>
          toImported(
            "bookmark",
            row.url,
            row.title,
            Math.floor(row.dateAdded / 1_000),
          ),
        );
        const history = historyRows.flatMap((row) =>
          toImported(
            ...
            row.url,
            row.title,
            Math.floor((row.last_visit_date ?? 0) / 1_000),
          ),
        );
        return { bookmarks, history };
      },
...
async function readChromiumBookmarks(
  ...
  const visit = (value: unknown): void => {
    if (
      results.length >= MAX_IMPORT_ITEMS ||
      !value ||
      typeof value !== "object"
    )
      return;
    ...
    if (item.type === "url" && typeof item.url === "string") {
      ...
      if (url)
        results.push({
          id: importedId("bookmark", url),
          title:
            typeof item.name === "string"
              ? item.name.slice(0, 2_000) || url
              : url,
          url,
          time: chromiumTime(item.date_added),
        });
    }
...
async function readStagedDatabase<T>(
  source: string,
  empty: T,
  read: (database: BetterSqliteConnection) => Promise<T>,
): Promise<T> {
  if (!(await exists(source))) return empty;
  const staging = await mkdtemp(join(tmpdir(), "moon-import-"));
  const copy = join(staging, "source.sqlite3");
  try {
    await copyFile(source, copy);
    for (const suffix of ["-wal", "-shm"] as const)
      if (await exists(`${source}${suffix}`))
        await copyFile(`${source}${suffix}`, `${copy}${suffix}`);
    const database = new BetterSqliteConnection(copy, {
      readonly: true,
      fileMustExist: true,
    });
...
function chromiumTime(value: unknown): number {
  const numeric =
    typeof value === "string" || typeof value === "number" ? Number(value) : 0;
  return Math.max(0, Math.floor(numeric / 1_000 - CHROMIUM_EPOCH_OFFSET_MS));
}
...
function importedId(kind: string, url: string): string {
  return `import-${kind}-${createHash("sha256").update(url).digest("hex").slice(0, 24)}`;
}
function toImported(
  kind: string,
  rawUrl: string,
  ...
  return url
    ? [
        {
          id: importedId(kind, url),
          title: rawTitle?.slice(0, 2_000) || url,
          url,
...
export function sanitizeImportedBookmarkTitle(value: string): string {
  const decodedOnce = value.replace(/&(amp|lt|gt|quot|#39);/gi, (entity) => {
