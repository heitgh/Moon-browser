...
contextBridge.exposeInMainWorld("moonBrowser", Object.freeze({
  ...
  listLocalProfiles: () => invoke("local-profile:list"),
  createLocalProfile: profile => invoke("local-profile:create", profile),
  updateLocalProfile: profile => invoke("local-profile:update", profile),
  openLocalProfile: id => invoke("local-profile:open", { id }),
  createGuestProfile: () => invoke("local-profile:create-guest"),
  getLocalProfileDeletionSummary: id => invoke("local-profile:delete-summary", { id }),
  deleteLocalProfile: request => invoke("local-profile:delete", request),
  discoverImportSources: () => invoke("import:discover"),
  selectManualImportSource: () => invoke("import:select-manual"),
  importBrowserProfile: selection => invoke("import:run", selection),
  importBookmarksHtml: () => invoke("import:bookmarks-html"),
  listWallpapers: () => invoke("wallpaper:list"),
  getWallpaper: id => invoke("wallpaper:get", { id }),
  importWallpaper: () => invoke("wallpaper:import"),
  replaceWallpaper: id => invoke("wallpaper:replace", { id }),
  updateWallpaper: update => invoke("wallpaper:update", update),
  removeWallpaper: id => invoke("wallpaper:remove", { id }),
  exportWallpaper: id => invoke("wallpaper:export", { id }),
  importMarkdownNote: () => invoke("note:import-markdown"),
  exportMarkdownNote: id => invoke("note:export-markdown", { id }),
  ...
  onFullscreenChanged: listener => subscribe("browser:fullscreen-changed", listener),
