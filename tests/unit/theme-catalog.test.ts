...
describe("ThemeCatalog", () => {
  it("normalizes builtin, user and .moontheme records into one ordered catalog", () => {
    const user = { id: "user-theme", name: "Meu tema", createdAt: 20, config: createDefaultCustomization().global };
  ...
  it("uses the preview selection as active without mutating source records", () => {
    const source = { id: "user-theme", name: "Meu tema", createdAt: 20, config: createDefaultCustomization().global };
